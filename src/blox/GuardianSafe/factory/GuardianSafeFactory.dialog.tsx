...
interface GuardianSafeFactoryDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  factoryAddress: Address
}

export default function GuardianSafeFactoryDialog({ open, onOpenChange, factoryAddress }: GuardianSafeFactoryDialogProps) {
  ...
  useEffect(() => {
    if (receipt && isSuccess) {
      ...
      try {
        ...
        // Try to find and decode the GuardianSafeCreated event
        for (const log of factoryLogs) {
          try {
            ...
            // Try to decode the log as our GuardianSafeCreated event
            ...
            // Check if this is our GuardianSafeCreated event
            if (decodedLog.eventName === 'GuardianSafeCreated') {
              const safeAddress = decodedLog.args.safeAddress as Address
              console.log('Found deployed GuardianSafe address:', safeAddress)
        ...
        console.warn('No GuardianSafeCreated event found. All transaction logs:', receipt.logs)
      } catch (err) {
    ...
    if (isSuccess && deployedAddress && !contractAdded && lastUsedParams.current) {
      try {
        const contractInfo: SecureContractInfo = {
          ...
          contractType: 'guardian-safe',
          contractName: 'GuardianSafe',
          safeAddress: lastUsedParams.current.safeAddress
        }
  ...
  const handleDeploy = async (params: {
    ...
    try {
      ...
      console.log('Deploying GuardianSafe via factory:')
      console.log('Factory Address:', factoryAddress)
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-[425px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Deploy GuardianSafe via Factory</DialogTitle>
          <DialogDescription>
            Deploy a new GuardianSafe contract using the factory contract.
          </DialogDescription>
        </DialogHeader>
            ...
            {isSuccess && deployedAddress && (
              <div className="flex flex-col items-center justify-center space-y-4 py-8">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Your GuardianSafe has been deployed successfully.
                  </p>
