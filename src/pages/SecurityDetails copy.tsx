...
import { SecurityWorkflowProvider, useSecurityWorkflowContext, ActionType } from '@/contexts/SecurityWorkflowContext'
...
// Add this new component for role-based action feedback
function ActionStatusInfo({ 
  action, 
  pendingTx = null,
  className = "",
  contractInfo,
  ...
  isRoleConnected: (address: string) => boolean
}) {
  useSecurityWorkflowContext()
  
  const getActionStatus = () => {
    // Check for signed but not broadcasted transactions
    const hasPendingBroadcast = signedTransactions.some(tx => {
      const matchesAction = {
        'transfer_ownership': 'OWNERSHIP_TRANSFER',
        'update_broadcaster': 'BROADCASTER_UPDATE',
        'update_recovery': 'RECOVERY_UPDATE',
        'update_timelock': 'TIMELOCK_UPDATE'
      }[action] === tx.metadata?.type;
      return matchesAction && !tx.metadata?.broadcasted;
    });

    console.log(`ActionStatusInfo for ${action}:`, {
      hasPendingBroadcast,
      signedTransactions: signedTransactions.map(tx => ({
        type: tx.metadata?.type,
        broadcasted: tx.metadata?.broadcasted
      }))
    });

    // If there's a transaction waiting to be broadcasted
    if (hasPendingBroadcast && pendingTx) {
      if (isRoleConnected(contractInfo.broadcaster)) {
        return (
          <div className="flex flex-col space-y-2">
            <div className="flex items-center text-emerald-600 dark:text-emerald-400">
              <Radio className="h-4 w-4 mr-2" />
              <span className="font-medium">Ready to Broadcast</span>
            </div>
            <p className="text-muted-foreground">
              You can broadcast this transaction using your broadcaster wallet.
            </p>
          </div>
        )
      }
      return (
        <div className="flex flex-col space-y-3">
          <div className="flex items-center text-amber-600 dark:text-amber-400">
            ...
            <span className="font-medium">Awaiting Broadcast</span>
          </div>
          <div className="space-y-2 text-muted-foreground">
            <p>Transaction is waiting for the broadcaster to process it.</p>
            {isRoleConnected(contractInfo.owner) && (
              <div className="flex items-start mt-1 bg-muted/50 rounded-md p-2">
                <Shield className="h-4 w-4 mr-2 mt-0.5 text-blue-500" />
                <p>As the owner, you have the option to approve or cancel this transaction directly through the contract.</p>
              </div>
            )}
            <div className="flex items-start mt-1 bg-muted/50 rounded-md p-2">
              <Timer className="h-4 w-4 mr-2 mt-0.5 text-purple-500" />
              <p>Once the time delay expires, the transaction can proceed without requiring the broadcaster.</p>
            </div>
          </div>
        </div>
      )
    ...
    if (!pendingTx) {
      // Initial state - no pending transaction
      if (action === 'transfer_ownership') {
          ...
          return (
            <div className="flex flex-col space-y-2">
              <div className="flex items-center text-emerald-600 dark:text-emerald-400">
                ...
                <span className="font-medium">Recovery Action Available</span>
              </div>
              <p className="text-muted-foreground">
                Using your recovery wallet, you can initiate an ownership transfer request.
              </p>
            </div>
          )
        }
        return (
          <div className="flex items-center text-muted-foreground">
            <Clock className="h-4 w-4 mr-2" />
            <span>Waiting for recovery wallet to initiate the transfer process</span>
          </div>
        )
      } 
      
      if (action === 'update_broadcaster') {
        if (isRoleConnected(contractInfo.owner)) {
          return (
            <div className="flex flex-col space-y-2">
              <div className="flex items-center text-emerald-600 dark:text-emerald-400">
                <Shield className="h-4 w-4 mr-2" />
                <span className="font-medium">Owner Action Available</span>
              </div>
              <p className="text-muted-foreground">
                As the owner, you can submit a request to update the broadcaster.
              </p>
            </div>
          )
        }
        return (
          <div className="flex items-center text-muted-foreground">
            <Shield className="h-4 w-4 mr-2" />
            <span>Only the owner can request a broadcaster update</span>
          </div>
        )
      }
      
      if (action === 'update_recovery' || action === 'update_timelock') {
        if (isRoleConnected(contractInfo.owner)) {
          return (
            <div className="flex flex-col space-y-2">
              <div className="flex items-center text-emerald-600 dark:text-emerald-400">
              ...
              <p className="text-muted-foreground">
                As the owner, you can update the {action === 'update_recovery' ? 'recovery address' : 'timelock period'} using a meta-transaction.
              </p>
            </div>
          )
        }
        return (
          <div className="flex items-center text-muted-foreground">
            <Shield className="h-4 w-4 mr-2" />
            <span>Only the owner can update the {action === 'update_recovery' ? 'recovery address' : 'timelock period'}</span>
          </div>
        )
      }
    } else {
      ...
      if (action === 'transfer_ownership') {
        if (isRoleConnected(contractInfo.owner)) {
          return (
            <div className="flex flex-col space-y-2">
              <div className="flex items-center text-emerald-600 dark:text-emerald-400">
                <Shield className="h-4 w-4 mr-2" />
                <span className="font-medium">Owner Approval Required</span>
              </div>
              <p className="text-muted-foreground">
                You can approve this ownership transfer using a meta-transaction.
              </p>
            </div>
          )
        }
        if (isRoleConnected(contractInfo.recoveryAddress)) {
          return (
            <div className="flex flex-col space-y-2">
              <div className="flex items-center text-emerald-600 dark:text-emerald-400">
                <Key className="h-4 w-4 mr-2" />
                <span className="font-medium">Recovery Approval Required</span>
              </div>
              <p className="text-muted-foreground">
                You can approve this ownership transfer using your recovery wallet.
              </p>
            </div>
          )
        }
        return (
          <div className="flex items-center text-muted-foreground">
            <Clock className="h-4 w-4 mr-2" />
            <span>Waiting for owner or recovery wallet approval</span>
          </div>
        )
      }
        ...
        if (isRoleConnected(contractInfo.owner)) {
          return (
            <div className="flex flex-col space-y-2">
              ...
              <p className="text-muted-foreground">
                You can approve this broadcaster update request.
              </p>
            </div>
          )
        }
    ...
    return (
      <div className="flex items-center text-muted-foreground">
        <Clock className="h-4 w-4 mr-2" />
        <span>No actions available for your current role</span>
      </div>
    )
  }
...
export function SecurityDetails() {
  ...
  // Add an effect to debug ownership transfer broadcast button
  useEffect(() => {
    console.log('Ownership transfer broadcast button state:', {
      hasPendingBroadcast: signedTransactions.some(tx => tx.metadata?.type === 'OWNERSHIP_TRANSFER' && !tx.metadata?.broadcasted),
      signedTransactions: signedTransactions.map(tx => ({
        type: tx.metadata?.type,
        broadcasted: tx.metadata?.broadcasted
      })),
      isRoleConnected: connectedAddress && contractInfo ? connectedAddress.toLowerCase() === contractInfo.broadcaster.toLowerCase() : false,
      isBroadcasting
    });
  }, [signedTransactions, connectedAddress, contractInfo, isBroadcasting]);
  ...
  // Add this after the transactions effect
  useEffect(() => {
    console.log('Current signed transactions:', signedTransactions);
    console.log('Transactions that appear to need broadcasting:', 
      signedTransactions.filter(tx => 
  ...
  // Add debounced broadcast handler
  const handleBroadcast = async (type: 'OWNERSHIP_TRANSFER' | 'BROADCASTER_UPDATE' | 'RECOVERY_UPDATE' | 'TIMELOCK_UPDATE') => {
    try {
      // Prevent multiple rapid clicks
      const now = Date.now()
      if (now - lastBroadcastTime < 2000) { // 2 second cooldown
        return
      }
      
      // Prevent multiple concurrent broadcasts
      if (isBroadcasting) {
        return
      }
  ...
  return (
    <SecurityWorkflowProvider
      ...
      pendingTransactions={contractInfo?.operationHistory || []}
    >
                    ...
                    {isLoadingHistory ? (
                      ...
                      <>
                        <div className="flex flex-col space-y-3">
                          <div className="flex items-start gap-4">
                            <div className="flex flex-col items-center">
                              ...
                              <div className="w-[2px] h-full bg-muted my-2" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <Button 
                              ...
                              {!pendingOwnershipTx && (
                                <p className="text-sm text-muted-foreground mt-2">
                                  {isRoleConnected(contractInfo.recoveryAddress) 
                                    ? "You can initiate an ownership transfer request as the recovery wallet."
                                    : "Only the recovery wallet can initiate a transfer request."}
                                </p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <Button 
                                onClick={() => setShowOwnershipDialog(true)}
                                className={`w-full flex items-center justify-center gap-2 ${signedTransactions.some(tx => tx.metadata?.type === 'OWNERSHIP_TRANSFER' && !tx.metadata?.broadcasted) && pendingOwnershipTx ? 'border-2 border-yellow-500 dark:border-yellow-600' : ''}`}
                                size="sm"
                                variant={signedTransactions.some(tx => tx.metadata?.type === 'OWNERSHIP_TRANSFER' && !tx.metadata?.broadcasted) && pendingOwnershipTx ? "default" : "outline"}
                                disabled={
                                  !signedTransactions.some(tx => tx.metadata?.type === 'OWNERSHIP_TRANSFER' && !tx.metadata?.broadcasted) || 
                                  !isRoleConnected(contractInfo.broadcaster) || 
                                  isBroadcasting ||
                                  !pendingOwnershipTx
                                }
                              ...
                              {pendingOwnershipTx && (
                                <p className="text-sm text-muted-foreground mt-2">
                                  {isRoleConnected(contractInfo.owner) 
                                    ? "You can approve this transfer using a meta-transaction as the owner."
                                    : isRoleConnected(contractInfo.recoveryAddress)
                                    ? "You can approve this transfer using your recovery wallet."
                                    : "Waiting for owner or recovery wallet approval."}
                                </p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-muted-foreground mt-2">
                                {signedTransactions.some(tx => tx.metadata?.type === 'OWNERSHIP_TRANSFER' && !tx.metadata?.broadcasted) && pendingOwnershipTx
                                  ? isRoleConnected(contractInfo.broadcaster)
                                    ? "You can broadcast this transaction as the broadcaster."
                                    : "Waiting for the broadcaster to process this transaction."
                                  : !pendingOwnershipTx
                                    ? "No pending ownership transfer to broadcast."
                                    : "No signed transaction available to broadcast."}
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <ActionStatusInfo 
                          ...
                          isRoleConnected={isRoleConnected}
                        />
                        ...
                        <div className="flex flex-col space-y-3">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {!pendingBroadcasterTx && (
                                <p className="text-sm text-muted-foreground mt-2">
                                  {isRoleConnected(contractInfo.owner) 
                                    ? "You can submit a request to update the broadcaster as the owner."
                                    : "Only the owner can request a broadcaster update."}
                                </p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <Button 
                                onClick={() => setShowBroadcasterDialog(true)}
                                className={`w-full flex items-center justify-center gap-2 ${signedTransactions.some(tx => tx.metadata?.type === 'BROADCASTER_UPDATE' && !tx.metadata?.broadcasted) && pendingBroadcasterTx ? 'border-2 border-yellow-500 dark:border-yellow-600' : ''}`}
                                size="sm"
                                variant={signedTransactions.some(tx => tx.metadata?.type === 'BROADCASTER_UPDATE' && !tx.metadata?.broadcasted) && pendingBroadcasterTx ? "default" : "outline"}
                                disabled={!signedTransactions.some(tx => tx.metadata?.type === 'BROADCASTER_UPDATE' && !tx.metadata?.broadcasted) || !isRoleConnected(contractInfo.broadcaster) || isBroadcasting}
                              >
                                <Shield className="h-4 w-4" />
                                Approve Update
                              </Button>
                              {pendingBroadcasterTx && (
                                <p className="text-sm text-muted-foreground mt-2">
                                  {isRoleConnected(contractInfo.owner)
                                    ? "You can approve this broadcaster update request as the owner."
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <Button 
                                ...
                                className={`w-full flex items-center justify-center gap-2 ${signedTransactions.some(tx => tx.metadata?.type === 'BROADCASTER_UPDATE' && !tx.metadata?.broadcasted) ? 'border-2 border-yellow-500 dark:border-yellow-600' : ''}`}
                                size="sm"
                              ...
                              <p className="text-sm text-muted-foreground mt-2">
                                {signedTransactions.some(tx => tx.metadata?.type === 'BROADCASTER_UPDATE' && !tx.metadata?.broadcasted)
                                  ? isRoleConnected(contractInfo.broadcaster)
                                    ? "You can broadcast this transaction as the broadcaster."
                                    : "Waiting for the broadcaster to process this transaction."
                                  : "No pending transactions to broadcast."}
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="flex flex-col space-y-3">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground mt-2">
                            {isRoleConnected(contractInfo.owner)
                              ? "You can update the recovery address using a meta-transaction."
                              : "Only the owner can update the recovery address."}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <Button 
                            ...
                            className={`w-full flex items-center justify-center gap-2 ${signedTransactions.some(tx => tx.metadata?.type === 'RECOVERY_UPDATE' && !tx.metadata?.broadcasted) ? 'border-2 border-yellow-500 dark:border-yellow-600' : ''}`}
                            size="sm"
                            variant={signedTransactions.some(tx => tx.metadata?.type === 'RECOVERY_UPDATE' && !tx.metadata?.broadcasted) ? "default" : "outline"}
                            disabled={!signedTransactions.some(tx => tx.metadata?.type === 'RECOVERY_UPDATE' && !tx.metadata?.broadcasted) || !isRoleConnected(contractInfo.broadcaster) || isBroadcasting}
                          >
                            {isBroadcasting ? (
                              <>
                                <Loader2 className="h-4 w-4 animate-spin" />
                    ...
                    <div className="flex flex-col space-y-3">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground mt-2">
                            {isRoleConnected(contractInfo.owner)
                              ? "You can update the timelock period using a meta-transaction."
                              : "Only the owner can update the timelock period."}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <Button 
                            ...
                            className={`w-full flex items-center justify-center gap-2 ${signedTransactions.some(tx => tx.metadata?.type === 'TIMELOCK_UPDATE' && !tx.metadata?.broadcasted) ? 'border-2 border-yellow-500 dark:border-yellow-600' : ''}`}
                            size="sm"
                            variant={signedTransactions.some(tx => tx.metadata?.type === 'TIMELOCK_UPDATE' && !tx.metadata?.broadcasted) ? "default" : "outline"}
                            disabled={!signedTransactions.some(tx => tx.metadata?.type === 'TIMELOCK_UPDATE' && !tx.metadata?.broadcasted) || !isRoleConnected(contractInfo.broadcaster) || isBroadcasting}
                          >
                            {isBroadcasting ? (
                              <>
                                <Loader2 className="h-4 w-4 animate-spin" />
